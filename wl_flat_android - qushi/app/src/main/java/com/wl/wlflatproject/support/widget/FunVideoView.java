package com.wl.wlflatproject.support.widget;
...
import com.wl.wlflatproject.support.FunError;
...
import com.wl.wlflatproject.support.FunPath;
...
//		DecoderManaer.SetEnableHDec(true);  //Open harddecode？
	...
	public GLSurfaceView getmSufaceView(){
		return mSufaceView;
	}
			...
			if ( null == mSufaceView
                ...
                if (fishEyeFrame instanceof SDK_FishEyeFrameSW) {
                    ...
                    if (((SDK_FishEyeFrameSW)fishEyeFrame).st_1_lensType
                        ...
                        ((VRSoftGLView) mSufaceView).setType(VRSoftDefine.XMVRType.XMVR_TYPE_180D);
                        ((VRSoftGLView) mSufaceView).setFecParams(GL2JNIView.FecType.GENERAL_180VR, fecCenter);
                    } else {
                        // 默认按照360VR处理
                        ((VRSoftGLView) mSufaceView).setType(VRSoftDefine.XMVRType.XMVR_TYPE_360D);
                        ((VRSoftGLView) mSufaceView).setFecParams(GL2JNIView.FecType.GENERAL_360VR, fecCenter);
                    }
...
//                    ((VRSoftGLView) mSufaceView).setCameraMount(VRSoftDefine.XMVRMount.Wall);
                    ...
                    if (fishEyeFrame instanceof SDK_FishEyeFrameCM) {
                        ((VRSoftGLView)mSufaceView).setType(VRSoftDefine.XMVRType.XMVR_TYPE_SPE_CAM01);
                    }
                }
				LayoutParams lp = new LayoutParams(
	                     LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
...
//                FunSDK.MediaSetFluency(mPlayerHandler, SDKCONST.EDECODE_TYPE.EDECODE_REAL_TIME_STREAM0, 0); //设置流畅度（实时<-->流畅）
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        // TODO Auto-generated method stub
		if(ev.getAction()==MotionEvent.ACTION_DOWN&&clickListener!=null){
